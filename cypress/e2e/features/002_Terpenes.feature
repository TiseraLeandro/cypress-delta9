@Terpenes @SANITY
Feature: Terpenes - Profile

    E2E test for Terpenes profile
    ...
    Scenario: Sample Creation For Terpenes Profile
        When Navigate to "Orders" in main menu
        And Click on the order in the grid view
        And Click on "Create" button in Sample section
        And User enter the sample data for "terpenes" profile
        And Click on "Create" button in Sample section
        And Click on the first sample for "terpenes" profile in Sample section
        Then A sample for "terpenes" profile is created
        And Sample status is "Collected" in Sample section
    
    Scenario: Receive Sample Partitions For Terpenes Profile
        When Navigate to "Orders" in main menu
        And Click on the order in the grid view
    ...
    Scenario: Accession Sample Partitions For Terpenes Profile
        When Navigate to "Orders" in main menu
        And Click on the order in the grid view
        And Click on the sample in Sample section
    ...
    Scenario: Set Up Worksheet For Terpenes Sample
        When Navigate to "Worksheets" in main menu
        And Click on "Set up worksheet" button in the header
        And Complete set up worksheet modal for "Terpenes" order
        And Click on "Set up worksheet" button in action modal
        Then A worksheet for "Terpenes" is created
        And Worksheet status is "In Progress"
       
    Scenario: Set up materials for Terpenes worksheet
        When Navigate to "Worksheets" in main menu
        And Click on the worksheet in the grid view
        And Click on "Add materials" button
        And Add "Tube" as material to the order
        Then Click on "Add materials" button in action modal
        And Materials table is completed
    
    Scenario: Set up instruments for Terpenes worksheet
        When Navigate to "Worksheets" in main menu
        And Click on the worksheet in the grid view
        And Click on "Add instruments" button
        And Add "Autosampler" "CTC Combi-PAL - TUSCADERO" as an instrument to the order
        Then Click on "Add instruments" button in action modal
        And Instrument "Autosampler" "CTC Combi-PAL - TUSCADERO" shows in the table
    
    Scenario: Start Preparation for Terpenes worksheet
        When Navigate to "Worksheets" in main menu
        And Click on the worksheet in the grid view
    ...
    Scenario: Publish Terpenes results
        When Navigate to "Orders" in main menu
        And Click on the order in the grid view
        And Click on the sample in Sample section
